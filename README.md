...
| Inspect | `hop_plugins`, `hop_metadata_types`, `hop_metadata_list`, `hop_metadata_get`, `hop_metadata_dependencies`, `hop_resolve_configuration`, `hop_definition_diff`, `hop_impact_analysis`, `hop_environment_diff`, `hop_catalog`, `hop_list_definitions`, `hop_inspect`, `hop_context`, `hop_component`, `hop_component_lineage`, `hop_read_text`, `hop_search`, `hop_find_table`, `hop_dependencies` | enabled |
...
Inspect the generated package with `unzip -l target/hop-mcp-connector-2.2.0.zip`; it must contain `plugins/misc/hop-mcp-connector/`, `LICENSE`, and `NOTICE`, and must not contain Hop runtime jars.
