...
| 2.2.0-SNAPSHOT (`main`) | 2.19.0 compile baseline; 2.20.0-SNAPSHOT profile | 21 | 2.0.1 | 2025-11-25 |
...
Build the Marketplace ZIP and CycloneDX SBOM with `mvn -B clean verify`. The published 2.1.0 release artifact is `hop-mcp-connector-2.1.0.zip`; current `main` builds `target/hop-mcp-connector-2.2.0-SNAPSHOT.zip`. SBOM files are `target/bom.json` and `target/bom.xml`. Install the ZIP into Hop's `plugins/misc/hop-mcp-connector/` directory or use the repository/catalog metadata in `marketplace/`. Restart Hop, then run `hop mcp --help`.
